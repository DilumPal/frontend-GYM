...
export default function Cart() {
    ...
    return (
        <div className="w-full min-h-screen h-auto flex flex-col items-center pt-4 pb-24 relative bg-primary">
            
            {/* Desktop Total Sticky Card */}
            <div className="hidden w-[300px] h-[120px] bg-primary border border-secondary/10 rounded-2xl shadow-2xl absolute bottom-1 md:top-4 right-4 md:flex flex-col justify-center items-center">
                ...
                <Link to="/checkout" state={{ cart: cart }} 
                    className="text-secondary bg-accent px-6 py-2 rounded-xl font-bold hover:bg-accent/80 transition-all duration-300 shadow-[0_4px_15px_rgba(229,57,53,0.3)]"
                >
                    Checkout
                </Link>
            </div>
            ...
            {cart.map((item) => {
                return (
                    <div key={item.productID} className="w-[70%] md:w-[600px] md:h-[100px] rounded-tl-3xl my-4 rounded-bl-3xl bg-secondary/[0.03] border border-secondary/10 shadow-2xl flex flex-col md:flex-row relative justify-center items-center p-2 md:p-0">
            ...
            <div className="z-50 w-full md:hidden h-[120px] bg-primary border-t border-secondary/10 shadow-2xl flex flex-col justify-center items-center mt-auto fixed bottom-0 left-0">
                <p className="text-2xl py-2 text-secondary font-bold">Total:
                    <span className="text-accent font-bold mx-2">
                        Rs {getTotal().toFixed(2)}
                    </span>
                </p>
